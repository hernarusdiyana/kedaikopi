<div class="bg-white p-4 rounded-md">
	...
	<div class="space-y-2">
		<div class="mb-4">
		<h2 class="text-base font-semibold leading-7 text-gray-900">Edit Produk Baru</h2>
    ...
    <!-- <form action="<?php echo base_url().'admin/product_update' ?>" method="POST"> -->
	<div class="">
		<div class="grid grid-cols-1 gap-x-6 gap-y-8 sm-grid-cols-6 space-y-2">
        <div class="flex flex-col gap-2">
            <label>Nama Produk</label>
            <input type="text" name="name" class="py-2 px-5 rounded-lg bg-white border" placeholder="Masukkan Nama Produk" value="<?= $product->name ?>" required>
        ...
        <div class="flex flex-col gap-2">
            <label>Harga</label>
            <input type="number" name="price" class="py-2 px-5 rounded-lg bg-white border" value="<?= $product->price ?>" required>
        </div>
		<div class="flex flex-col gap-2">
			<label for="image">Gambar Produk</label>
			<input type="file" name="image" value="<?= base64_encode($product->image) ?>" required>
			<img src="data:image/jpeg;base64,<?= base64_encode($product->image) ?>" alt="Gambar Produk" class="w-24 h-24 object-cover" loading="lazy">	
		</div>
